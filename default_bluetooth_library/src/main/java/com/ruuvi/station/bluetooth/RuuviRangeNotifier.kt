...
class RuuviRangeNotifier(
    ...
    private fun sendDataToListener(tag: FoundSensor) {
        if (tag.measurementSequenceNumber != null) {
            val lastSequenceNumber = sequenceMap[tag.id]

            if (lastSequenceNumber == 1 || tag.measurementSequenceNumber != lastSequenceNumber) {
    ...
    private fun getScanFilters(): List<ScanFilter>? {
        ...
        val ruuviFilter = ScanFilter
                ...
                .build()
        val Mysensor = ScanFilter
            .Builder()
            .setDeviceAddress("Test")
            .build()
